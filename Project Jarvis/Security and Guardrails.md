...
- **Audit everything:** log every tool call (who/what/when/which params) for after-the-fact review. Implemented in `server.py` (the vault's local chat/capture/voice server, not the planned production orchestrator) as of 2026-08-13: `AuditLog` appends one JSON line per `/chat`, `/remember`, `/speak` request to `audit.log.jsonl`, including denials from the auth/rate-limit gates below. Rate limiting and auth on those routes were added the same day: a shared-secret token (`AuthGate`, `JARVIS_TOKEN` in a git-ignored `.env`) required on the `X-Jarvis-Token` header, plus a per-IP fixed-window rate limiter (`RateLimiter`, 30 req/60s). See [[Phase 0 Progress Tracker]] checklist, now closed.
